...
public partial class TemperatureEntryViewModel(EntryRepository<TemperatureEntry> repository, CurrentChildContext childContext) : ObservableObject
{
    ...
    [ObservableProperty] private double _value = 36.6;
    ...
    public void LoadEntry(TemperatureEntry? entry)
    {
        ...
        Value = (double)entry.ValueCelsius;
        Notes = entry.Notes ?? "";
    }
    ...
    private async Task Save()
    {
        ...
        try
        {
            ...
            var celsius = (decimal)Value;

            if (_entryId is { } id)
            {
                await repository.UpdateAsync(new TemperatureEntry
                {
                    Id = id,
                    ChildId = childId,
            ...
            {
                await repository.AddAsync(new TemperatureEntry
                {
                    ChildId = childId,
                    OccurredAt = occurredAt,
                    ValueCelsius = celsius,
